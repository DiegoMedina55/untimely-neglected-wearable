...
def get_smart_moves(possible_moves, body, board, my_snake):
    ...
    if (len(smart_moves) > 1 or my_snake['head'] in board['hazards']) and board['food'] and not eating_snakes and (my_snake["health"] < hunger_threshold or any(snake["length"] + len(board['food']) >= my_snake["length"] for snake in enemy_snakes)):
        print("Hungry!")
...
#                        minimum_moves_threshold = 6
    ...
    if len(smart_moves) > 1:
        if tron_mode:
            center_target = get_moves_toward(my_snake['head'], {'x':(board['width']-1)/2, 'y':(board['height']-1)/2})
            test_moves = [move for move in smart_moves if move in center_target]
            if test_moves:
                smart_moves = test_moves
        else:
